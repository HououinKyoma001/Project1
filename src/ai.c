...
int trapVerdict(Sprite* sprite) {//用于判断角色是否受到陷阱的影响并返回相应的值。
  int ret = 0;
...
int getPowerfulPlayer() {//用于获取游戏中最强大的玩家的ID，即拥有最高分数的玩家。
...
int balanceVerdict(Sprite* sprite, int id) {//根据输入的玩家ID判断角色是否要保持平衡，并返回相应的值。
  if (id == -1) return 0;
...
int testOneMove(Snake* snake, Direction direction) {//测试角色在给定的方向上移动一步后，返回该操作的价值（根据陷阱、碰撞等因素计算）。
                                                                                 //该函数会预测多个步数后的情况。
...
int compareChoiceByValue(const void* x, const void* y) {//用于比较选择的价值，用于排序。
  const Choice *a = x, *b = y;
  return b->value - a->value;
}

void AiInput(Snake* snake) {                                    //根据当前情况和测试的结果，选择合适的方向作为AI角色的输入，并改变角色的移动方向。
  Sprite* snakeHead = snake->sprites->head->element;
