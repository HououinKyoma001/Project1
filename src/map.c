...
bool isTrap[MAP_SIZE][MAP_SIZE];//表示地图上每个格子是否是一个陷阱。
bool primMap[MAP_SIZE][MAP_SIZE];//表示地图上每个格子是否是一个主要路径块。
bool hasMap[MAP_SIZE][MAP_SIZE];//表示地图上每个格子是否已经生成了地图元素。
int exitX, exitY;//表示地图的出口在地图上的坐标。
void clearMapGenerator() {//清除地图生成器的状态，初始化地图生成器的相关变量。
  exitX = exitY = -1;
  memset(hasMap, 0, sizeof(hasMap));
  memset(isTrap, 0, sizeof(isTrap));
  memset(primMap, 0, sizeof(primMap));
}
int count(int x, int y) {       //计算地图上位置 (x, y) 周围相邻格子中主要路径块的数量。
  int ret = 0;
...
#endif
void initPrimMap(double floorPercent, int smoothTimes) {//该函数用于初始化主要路径块数组（primMap）。
                                                         // 函数接受两个参数：floorPercent 和 smoothTimes。
  memset(primMap, 0, sizeof(primMap));
